...
class 'HandheldRailgun' (Weapon)

HandheldRailgun.kMapName = "HandheldRailgun"

local kChargeTime = 2
// The HandheldRailgun will automatically shoot if it is charged for too long.
local kChargeForceShootTime = 3
local kHandheldRailgunRange = 400
...
function HandheldRailgun:OnCreate()
...
function HandheldRailgun:OnDestroy()
...
function HandheldRailgun:OnPrimaryAttack(player)
...
function HandheldRailgun:GetWeight()
    return kHandheldRailgunWeight
end

function HandheldRailgun:OnPrimaryAttackEnd(player)
    self.railgunAttacking = false
end

function HandheldRailgun:GetBarrelPoint()
...
function HandheldRailgun:GetTracerResidueEffectName()
    return kRailgunTracerResidueEffectName
end

function HandheldRailgun:GetTracerEffectFrequency()
    return 1
end

function HandheldRailgun:GetDeathIconIndex()
    return kDeathMessageIcon.Railgun
end

function HandheldRailgun:GetChargeAmount()
...
function HandheldRailgun:GetIsAffectedByWeaponUpgrades()
    return true
end
...
local function ExecuteShot(self, startPoint, endPoint, player)
    ...
    local damage = kHandheldRailgunDamage + math.min(1, (Shared.GetTime() - self.timeChargeStarted) / kChargeTime) * kHandheldRailgunChargeDamage
...
local function Shoot(self, leftSide)
    ...
    if player then
        ...
        local spreadDirection = CalculateSpread(shootCoords, kHandheldRailgunSpread, NetworkRandom)
        
        local endPoint = startPoint + spreadDirection * kHandheldRailgunRange
...
if Server then
    ...
    function HandheldRailgun:OnParentKilled(attacker, doer, point, direction)
    end
    
    /**
     * The HandheldRailgun explodes players. We must bypass the ragdoll here.
     */
    function HandheldRailgun:OnDamageDone(doer, target)
...
function HandheldRailgun:ProcessMoveOnWeapon(player, input)
...
function HandheldRailgun:OnUpdateRender()

    PROFILE("HandheldRailgun:OnUpdateRender")
...
function HandheldRailgun:OnTag(tagName)
...
function HandheldRailgun:OnUpdateAnimationInput(modelMixin)
...
if Client then
    ...
    local kHandheldRailgunMuzzleEffectRate = 0.5
    ...
    function HandheldRailgun:GetIsActive()
        return true
    end
    
    function HandheldRailgun:GetPrimaryEffectRate()
        return kHandheldRailgunMuzzleEffectRate
    end
    
    function HandheldRailgun:GetPrimaryAttacking()
        return (Shared.GetTime() - self.timeOfLastShot) <= kHandheldRailgunMuzzleEffectRate
    end
    
    function HandheldRailgun:GetSecondaryAttacking()
        return false
    end
    
    function HandheldRailgun:OnClientPrimaryAttacking()
    ...
    function HandheldRailgun:OnProcessMove(input)
        ...
        if player then
            ...
            local startPoint = player:GetEyePos()
            local endPoint = startPoint + player:GetViewCoords().zAxis * kHandheldRailgunRange
            local trace = Shared.TraceRay(startPoint, endPoint, CollisionRep.Damage, PhysicsMask.Bullets, EntityFilterAllButIsa("Tunnel"))
            local direction = (endPoint - startPoint):GetUnit()
            
            local extents = GetDirectedExtentsForDiameter(direction, kBulletSize)
            
            self.HandheldRailgunTargetId = nil
            ...
            if trace.fraction < 1 then
                ...
                for i = 1, 20 do
                    ...
                    if capsuleTrace.entity then
                        ...
                        capsuleTrace.entity:SetHandheldRailgunTarget()
                        self.HandheldRailgunTargetId = capsuleTrace.entity:GetId()
                        break
    ...
    function HandheldRailgun:GetTargetId()
        return self.HandheldRailgunTargetId
    end
    
end

Shared.LinkClassToMap("HandheldRailgun", HandheldRailgun.kMapName, networkVars)