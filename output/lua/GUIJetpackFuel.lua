...
GUIJetpackFuel.kBackgroundOffsetX = 6
...
function GUIJetpackFuel:Update(deltaTime)
	...
	if player then 
		local showFuelBar = (HasMixin(player, "MagnoBootsWearer") and player:GetHasMagnoBoots()) or player:isa("JetpackMarine")
		self.background:SetIsVisible(showFuelBar)
	
		if showFuelBar and player.GetFuel then
